<template>
	...
	import { computed, inject, onMounted, ref, shallowRef, Ref, defineAsyncComponent, reactive } from 'vue';
	...
	const clipButton = shallowRef<HTMLElement>();
		const stealButton = shallowRef<HTMLElement>(); // ebisskey
	...
	function renote(viaKeyboard = false) {
		...
		if(["public", "home", "followers"].includes(props.note.visibility)){
			items = items.concat([{
	...
	function addStar(ev?: MouseEvent): void {
		pleaseLogin();
	...
	function undoStar(ev?: MouseEvent): void {
		pleaseLogin();
		const oldStar = note.myReaction;
		if (!oldStar) return;
